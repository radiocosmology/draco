        ...
        for lfi in range(ssiv.shape[freq_axis]):
            ...
            gx_dirty[..., 1] = (
                ...
                (
                    ssiw[fslice]
                    * ssiv[fslice]
                    * np.gradient(sssv[fslice] * ns_matrix, axis=ns_axis)
                )
                .reshape(newshape)
                .sum(axis=ra_axis)
            )
            ...
            mNi[..., 1, 1] = (
                (
                    np.gradient(sssv[fslice] * ns_matrix, axis=ns_axis) ** 2
                    * ssiw[fslice]
                )
                .reshape(newshape)
                .sum(axis=ra_axis)
...
class ExtractFocalCrosstalkGain(task.SingleTask):
    ...
    def process(
        self, ss: containers.SiderealStream
    ) -> containers.VisFocalCrosstalkGain:
        ...
        for lfi in range(ssiv.shape[freq_axis]):
            ...
            gx_dirty[..., 1] = (
                (
                    np.gradient(sssv[fslice] * ns_matrix, axis=ns_axis).conj()
                    * ssiw[fslice]
                    * ssiv[fslice]
                )
                .reshape(newshape)
            ...
            mNi[..., 1, 1] = (
                (
                    np.abs(np.gradient(sssv[fslice] * ns_matrix, axis=ns_axis)) ** 2
                    * ssiw[fslice]
                )
                .reshape(newshape)
                .sum(axis=ra_axis)
