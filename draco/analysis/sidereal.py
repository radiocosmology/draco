...
from ..util import gpr, regrid, tools
    ...
    def process(self, data):
        ...
        if "time" in data.index_map:
            ...
            raise TypeError(
                ...
                "Expected container with a `time` or a `ra` axis."
            )
...
class SiderealRegridderGPR(SiderealRegridder):
    ...
    ----------
    mask_cutoff : float
        Distance (in number of samples) from nearest input sample to
        keep interpolated output samples. Default is 1.5.
    """

    mask_cutoff = config.Property(proptype=float, default=1.9)
    ...
    def _regrid(self, vis, weight, times):
        ...
        kernel_params = {
            "name": "matern",
            "width": self.kernel_width * np.median(abs(np.diff(grid))),
            "alpha": 1.0,
            "nu": 1.5,
            "svar": self.snr_cov,
        }
        ...
        # Resample
        vout, wout = gpr.resample(
            vx,
            wx,
            xi=times,
            xo=grid,
            cutoff_dist=self.mask_cutoff,
            flagn=self.mask_cutoff_partition,
            **kernel_params,
        )
