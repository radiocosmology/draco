...
class CatalogPixelization(task.SingleTask):
    ...
    def process(self, rm, catalog):
        ...
        rm_freq = rm.index_map["freq"][:]

        freq_axis_index = list(rm.map.attrs["axis"]).index("freq")
        nfreq_local = rm.map.local_shape[freq_axis_index]
        freq_local_offset = rm.map.local_offset[freq_axis_index]

        local_freq_slice = slice(
            freq_local_offset, freq_local_offset + nfreq_local
        )

        rm_ra = rm.index_map["ra"][:]
        rm_el = rm.index_map["el"][:]

        pos_arr = np.array(catalog["position"])
        dec_arr = pos_arr["dec"]
        ra_arr = pos_arr["ra"]

        freq_arr = NU21 / (1 + catalog["redshift"]["z"])
