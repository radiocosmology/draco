...
class GeneralInputContainerMixin:
    ...
    axis or the `correlated_axis`. These constituent axes are included in the index map,
    ...
    dataset = config.Property(proptype=str, default=None)
    correlated_axis = config.Property(proptype=str)
    ...
    def _prepare_inputs(self, ss):
        ...
        if (
            self.correlated_axis not in ss.axes
            or self.correlated_axis not in data_dset.attrs["axis"]
        ):
            raise ValueError(
                f"Specified correlated axis ({self.correlated_axis}) not in "
                f"container of type {type(ss)}."
            )

        # Find the relevant axis positions
        data_view, bl_axes = flatten_axes(data_dset, [self.correlated_axis, "freq"])
        weight_view, _ = flatten_axes(
            ss.weight, [self.correlated_axis, "freq"], match_dset=data_dset
        )

        return data_view, weight_view, bl_axes


class DelayPowerSpectrumContainerMixin:
...
class DelayTransformContainerMixin:
    ...
    def _create_output(
        ...
        delay_spec = containers.DelayTransform(
            baseline=nbase,
            sample=ss.index_map[self.correlated_axis],
            delay=delays,
            attrs_from=ss,
...
class DelayTransformFFT(
    ...
    def setup(self):
        """Initialize a different output container depending on the output."""
        if self.powerspectrum:
            self._create_output = partial(
                DelayPowerSpectrumContainerMixin._create_output, self
            )
            # This is a bit of a workaround to support creating
            # a power spectrum in the delay transform code. I
            # should try to clean it up
            self.nsamp = 1
            self.save_samples = False
            self.save_spectrum_mask = False
        else:
            self._create_output = partial(
                DelayTransformContainerMixin._create_output, self
            )
...
class DelayTransformWienerFilter(DelayTransformFFT):
...
class DelayPowerSpectrumBase(
    GeneralInputContainerMixin, DelayPowerSpectrumContainerMixin, DelayTransformBase
):
...
class DelayPowerSpectrumGibbs(DelayPowerSpectrumBase, random.RandomTask):
    ...
    def _estimator(self, data, weight, S, ndelay, channel_ind):
        """Use a gibbs sample to calculate a power spectrum."""
...
class DelayCrossPowerSpectrumEstimator(DelayPowerSpectrumGibbs, random.RandomTask):
    ...
    def _prepare_inputs(
        ...
        for ss in sslist:
            ...
            dv, wv, ca = GeneralInputContainerMixin._prepare_inputs(self, ss)
...
def delay_spectrum_wiener_filter(
    ...
    # and transpose to [average_axis, delay]
...
def DelayPowerSpectrumStokesIEstimator(*args, **kwargs):
    """Function to handle deprecated class."""
...
def DelayPowerSpectrumGeneralEstimator(*args, **kwargs):
    """Function to handle deprecated class."""
    raise DeprecationWarning(
