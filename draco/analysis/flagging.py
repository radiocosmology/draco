...
from caput import config, fftw, mpiarray, weighted_median
...
class RFITransientVisMask(RFIVisMask):
    ...
    def generate_mask(self, vis, weight, mask, freq, baselines, times):
        ...
        valid_freqs = np.any(mask, axis=-1)
        ...
        mhpf = fftw.fft(vhpf, axes=1)
        # Apply the absolute value operation in place to save memory
        np.absolute(mhpf, out=mhpf)

        # Create the output mask. The shape is dependent on whether
        # or not we reduce over baselines in this task.
        ...
        for ii, fsel in enumerate(np.flatnonzero(valid_freqs)):
            ...
            mad_ = mad(mhpf[ii], fl[fsel], self.mad_base_size, self.mad_dev_size)
        ...
        # won't wave regions which are wide in _both_ time and frequency, so this
        # should have a fairly minimal effect on the total flagging.
        fl |= rfi.sir(fl, eta=0.2)

        # TODO: some sort of similar consensus operator across frequency?
