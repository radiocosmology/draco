...
class ApplyTimeFreqMask(task.SingleTask):
    ...
    def process(self, tstream, rfimask):
        ...
        if self.match_axes:
            ...
            # Using `isin` returns a boolean array, which is
            # preferrable for writing compared to something
            # like `intersect1d`, which returns the indices.
        ...
        bcast_slice[m_axes.index(timelike_ax)] = mask_sel
        ...
        # Mask the data. For future reference, `inp_slice` will only work
        # correctly if
        # - all entries in `inp_slice` are slice objects
        # - `inp_slice` is a boolean array with the same shape as `weight`
        # - only a single axis in indexed (this is the case we're in)
        tsc.weight[:].local_array[inp_slice] *= (~mask[bcast_slice]).astype(np.float32)
