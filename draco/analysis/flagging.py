...
class RFIStaticVisMask(RFIVisMask):
    ...
    rho
        ...
        Default is 2.0.
    sigma_high
        Median absolute deviations sigma threshold. Default is 5.0.
    sigma_low
        Median absolute deviations low sigma threshold. A value above
        this threshold is masked only if it is either larger than `sigma_high`
        or it is larger than `sigma_low` AND connected to a region larger
        than `sigma_high`. Default is 3.0.
    eta
    ...
    sigma_high: float = config.Property(proptype=float, default=5.0)
    sigma_low: float = config.Property(proptype=float, default=3.0)
    ...
    def generate_mask(self, vis, weight, mask, freq, baselines, times):
        ...
        if self.beamform:
            # fft always returns a numpy array, so have to re-wrap
            vis = np.fft.fft(vis.local_array, axis=1)
        ...
        for bsize in self.bins:
            ...
            # TODO: this threshold has to change based on the baseline length
            # Can we use the weights for that?
