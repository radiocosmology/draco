...
class ReduceMaskEl(task.SingleTask):
    ...
    def process(self, stream):
        ...
        Parameters
        ----------
        stream : containers.LocalizedRFIMask(freq, el, time) or containers.SiderealLocalizedRFIMask(freq, ra, el)
        ...
        if (not isinstance(stream, containers.LocalizedRFIMask)) & (
            not isinstance(stream, containers.LocalizedSiderealRFIMask)
        ):
            raise ValueError(
                f"Input class must be LocalizedRFIMask or LocalizedSiderealRFIMask. Got {type(stream)}."
            )

        # Extract mask/frac and axes data
        mask = stream.mask[:]
        el_axis = list(stream.mask.attrs["axis"]).index("el")
        freq = stream.freq[:]
        ...
        if isinstance(stream, containers.LocalizedRFIMask):
            # LocalizedRFIMask(freq, el, time) -> RFIMask(freq, time)
            time = stream.time[:]
            output = containers.RFIMask(freq=freq, time=time)
        elif isinstance(stream, containers.LocalizedSiderealRFIMask):
...
class ApplyLocalizedRFIMask(task.SingleTask):
    ...
    def process(self, tstream, rfimask):
        ...
        # Find the overlapping indices for freq, ra, and el
        freq_overlap = np.intersect1d(tstream.freq, rfimask.freq, return_indices=True)
        ...
        if len(freq_overlap[0]) == 0:
            raise ValueError("No overlapping freq regions found.")
        ...
        tsc.weight[:].local_array[
            np.ix_(pol_index, t_freq_index, t_ra_index, t_el_index)
        ] *= (~mask[np.ix_([0], m_freq_index, m_ra_index, m_el_index)]).astype(
            np.float32
        )
