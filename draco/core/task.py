...
def group_tasks(*tasks):
    ...
    class TaskGroup(*tasks):
        ...
        def setup(self, *args):
            ...
            Assumes that _all_ setup arguments have been provided, so dispatch in order
            based on the number of input arguments that each subtask takes.

            If two subtasks take the same input argument, it must be provided multiple times.

            At the moment, this doesn't try to figure out the arguments by name. Hopefully this
