...
class SingleTask(MPILoggedTask, pipeline.BasicContMixin):
    ...
    def finish(self):
        ...
        self.log.info(f"Starting finish for task {self.__class__.__name__}")
        ...
        try:
            ...
            self.log.info(f"Leaving finish for task {self.__class__.__name__}")

            return output

        except AttributeError:
            self.log.info(f"No finish for task {self.__class__.__name__}")
            pass
