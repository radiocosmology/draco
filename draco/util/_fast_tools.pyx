...
cpdef _invert_no_zero(real_or_complex [:] array, real_or_complex [:] out):
    ...
    if (real_or_complex is cython.doublecomplex) or (real_or_complex is cython.floatcomplex):
        for i in prange(n, nogil=True):
            cond = (array[i].real < thresh) if (array[i].real >= 0.) else (-array[i].real < thresh)
            cond = cond and ((array[i].imag < thresh) if (array[i].imag >= 0.) else (-array[i].imag < thresh))
            out[i] = 0. if cond else 1. / array[i]
    else:
        for i in prange(n, nogil=True):
            cond = (array[i] < thresh) if (array[i] >= 0.) else (-array[i] < thresh)
            out[i] = 0. if cond else 1. / array[i]