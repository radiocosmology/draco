...
def estimate(
    A: np.ndarray, b: np.ndarray, dtype: np.dtype, out: np.ndarray = None
) -> np.ndarray:
    ...
    """
    return np.matmul(
        A.astype(dtype), b.astype(dtype), dtype=dtype, out=out, casting="no"
    )


def inpaint(
    A: np.ndarray, b: np.ndarray, x: np.ndarray, W: np.ndarray, out: np.ndarray = None
) -> np.ndarray:
    """Inpaint using a DPSS basis and a set of coefficients."""
    # Make the inpainted data
    xinp = estimate(A, b, dtype=x.dtype, out=out)
