...
def resample(
    ...
    **kernel_params,
    ...
    # Scale the kernel width by the sample width
    kwidth = kernel_params.pop("width", 1.0)
    kernel_params.update({"width": kwidth * np.median(np.abs(np.diff(xo)))})

    # Create the source kernel and add a constant noise variance term
    Ki = kernels.get_kernel(name=kernel_type, N=xi, **kernel_params)
    np.einsum("ii->i", Ki)[:] += svar
    # Create the projection kernel
...
def interpolate_unweighted(
    ...
    which = "lower" if _solveh else "full"

    def solver(ab, b):
        if _solveh:
            # kernel is hermitian positive-definite, so use
            # the fastest solver possible
    ...
    for ii in range(data.shape[0]):
        ...
        kb = kernels.get_band_diagonal(K[mi][:, mi], which=which)
